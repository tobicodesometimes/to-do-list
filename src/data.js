...
// display seed data into our UI shape (id, text, completed, isEditing, draft)
const normalized = seedTodos.map(t => ({
  id: t.id,
  text: t.title,
  completed: false,
  isEditing: false,
...
const nextId = Math.max(...normalized.map(t => t.id), 0) + 1;

// Initial state consumed by useReducer:
export const initialState = {
  newText: "",
  nextId,
  // newest should appear on top => .reverse() so later seeds show first if desired
  todos: [...normalized]
};
